...
function SignUp() {
  ...
  const checkEnrollmentPeriod = async () => {
    try {
      setEnrollmentPeriodCheck(prev => ({ ...prev, loading: true, error: null }));
    ...
    try {
      ...
      for (let [key, value] of formData.entries()) {
        if (key !== 'validIdPath' && key !== 'idPhotoPath' && 
            !['contactNumber', 'altContactNumber', 'motherContact', 'fatherContact', 'guardianContact'].includes(key)) {
        ...
        <div className="relative max-w-full mx-auto bg-white-yellow-tone w-11/12 px-8 py-4 mt-8 mb-12 flex flex-col rounded-lg">
          ...
          {enrollmentPeriodCheck.loading && (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-lg text-gray-600">Checking enrollment availability...</p>
            </div>
          )}

          {/* No Ongoing Enrollment Period */}
          {!enrollmentPeriodCheck.loading && !enrollmentPeriodCheck.hasOngoingPeriod && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="mb-8">
                <svg className="w-24 h-24 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
                </svg>
                <h1 className="text-4xl font-bold text-gray-800 mb-4">Enrollment Currently Unavailable</h1>
                  ...
                  {enrollmentPeriodCheck.error && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                      <div className="flex items-center">
                        ...
                        <p className="text-red-700">{enrollmentPeriodCheck.error}</p>
                      </div>
                    </div>
                  )}

                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
                    <h3 className="text-lg font-semibold text-blue-800 mb-3">What can you do?</h3>
                    <ul className="text-left text-blue-700 space-y-2">
                      ...
                      <li className="flex items-start">
                        ...
                        Contact us directly for information about upcoming enrollments.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-green-700">
                        <strong>{enrollmentPeriodCheck.currentPeriod.periodName}</strong> - {enrollmentPeriodCheck.currentPeriod.batchName}
                      </p>
                      <p className="text-sm text-green-600">
                        Ends: {new Date(enrollmentPeriodCheck.currentPeriod.endAt).toLocaleDateString('en-US', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric'
                        })}
