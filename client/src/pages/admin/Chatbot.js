...
export default function Chatbot() {
  ...
  return (
    <div className="h-screen overflow-hidden flex bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      {/* Sidebar */}
      {sidebarOpen && (
        <aside className="w-64 h-full border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 flex flex-col">
          ...
          <div className="flex-1 overflow-auto">
            {chats.map((chat) => (
              <div
                key={chat.id}
                className={`flex items-center justify-between px-4 py-2 text-sm truncate cursor-pointer ${activeChat === chat.id
                    ? "bg-german-red bg-opacity-10 text-dark-red border-l-4 border-dark-red"
                    : "hover:bg-gray-100 dark:hover:bg-gray-700"
                  }`}
              >
                <button className="flex-1 text-left" onClick={() => setActiveChat(chat.id)}>
                  {chat.title}
                </button>
                <button
      ...
      <div className="flex-1 h-full overflow-hidden flex flex-col">
        ...
        <main className="flex-1 overflow-auto p-6 space-y-4">
                ...
                {quickActions.map((action) => (
                  ...
                  >
                    <div className="flex-1">
                      <span className="text-gray-700 dark:text-gray-300 font-medium text-sm leading-tight">{action.text}</span>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {messages.map((m) => (
            <div
              key={m.id}
              className={`flex items-start gap-2 max-w-xl ${m.role === "user" ? "ml-auto flex-row-reverse text-right" : "mr-auto text-left"}`}
            >
              {/* Avatar */}
              {m.role === "assistant" ? (
                <div className="flex items-center justify-center w-10 h-10 min-w-[2.5rem] min-h-[2.5rem] rounded-full bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 flex-shrink-0">
                  <svg width="20" height="20" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000">
                    <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                    <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                      <g id="🔍-Product-Icons" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                        <g id="ic_fluent_bot_24_filled" fill="#FD0100" fillRule="nonzero">
                ...
                <div className="flex items-center justify-center w-10 h-10 rounded-full bg-slate-600 text-white text-sm font-semibold flex-shrink-0">{getUserInitials()}</div>
              )}

              {/* Message bubble */}
              <div>
                <div
                  className={`inline-block px-4 py-2 rounded-2xl shadow-sm ${m.role === "user"
                      ? "bg-slate-600 text-white"
                      : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
                    }`}
                >
                  <p className="whitespace-pre-wrap text-base leading-relaxed">
                    {m.text || (isStreaming && m.role === "assistant" ? "…" : "")}
                  </p>
                </div>
                <div className="text-xs text-gray-400 mt-1 flex gap-2 items-center">
                  ...
                  {m.role === "assistant" && m.text &&
                    ...
                    !m.text.includes("Which class or semester would you like") &&
                    ...
                    !m.text.match(/^(I can|I'll|Let me help|I'm here to).*\?$/) && (
                      <button
        ...
        <footer className="flex-shrink-0 px-6 py-4 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800">
          <div className="flex items-center gap-2">
            <textarea
              ...
              placeholder={isStreaming ? "Assistant is typing…" : "Type something..."}
              rows={1}
