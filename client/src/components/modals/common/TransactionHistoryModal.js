    ...
    <Flowbite theme={{ theme: customModalTheme }}>
      ...
      >
        <div className="pt-6 flex flex-col bg-white-yellow-tone rounded-lg transition duration-150 ease-out">
          ...
          <Modal.Body>
            <div class="max-h-[600px] min-h-[400px]">
                ...
                <table class="table-fixed text-base text-left rtl:text-right text-black mx-auto w-full">
                  ...
                  <tbody className="text-center">
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan="7" className="px-2 py-4 text-red-600">
                          Error: {error}
                        </td>
                      </tr>
                    ) : assessmentData &&
                      assessmentData.payments &&
                      assessmentData.payments.length > 0 ? (
                      assessmentData.payments.map((payment, index) => {
                        // Format feeType (or paymentType) to be human-readable
                        let type =
                          payment.feeType || payment.paymentType || "-";
                        if (type && type !== "-") {
                          type = type
                            .replace(/_/g, " ")
                        ...
                        return (
                          <tr key={index} className="text-sm">
                            ...
                            <td className="px-2 py-4 uppercase break-all ">
                              {" "}
                              {payment.paymentMethod}{" "}
                            </td>
                            <td className="px-2 py-4 break-all text-wrap">
                              {" "}
