...
const FORM_CONFIG = {
  heights: {
    ...
    textareaWithoutAttachments:
      "h-[180px] sm:h-[200px] lg:h-[220px] mb-3 sm:mb-4",
    attachmentsContainer: "h-24 sm:h-28 lg:h-32",
  },
  styles: {
    input:
      "rounded-lg border-0 bg-[#F5F5F5] py-2.5 sm:py-3 px-4 sm:px-5 text-sm sm:text-base placeholder:text-[#575757] focus:outline-none focus:ring-0",
    textarea:
      "bg-transparent border-0 size-full resize-none py-0 ps-0 text-sm sm:text-base placeholder:text-[#575757] focus:outline-none focus:ring-0",
    contentContainer:
      "mt-2 sm:mt-3 mb-1 sm:mb-2 rounded-lg border-0 bg-[#F5F5F5] py-3 sm:py-4 px-4 sm:px-5",
  },
};
...
const getTextareaHeight = (hasAttachments) =>
  hasAttachments
    ? FORM_CONFIG.heights.textareaWithAttachments
    : FORM_CONFIG.heights.textareaWithoutAttachments;

const CloseButton = ({ onClick, className }) => (
  <button
    className={`ml-2 sm:ml-3 mr-auto inline-flex items-center rounded-lg p-1.5 text-sm text-black hover:bg-grey-1 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white ${className}`}
    onClick={onClick}
  >
    <svg
...
const ImagePreview = ({ src, onRemove }) => (
  <div className="flex-none relative w-16 h-16 sm:w-[70px] sm:h-[70px] lg:w-[75px] lg:h-[75px]">
    ...
    <img
      src={src}
      alt=""
      className="h-full w-full rounded-xl sm:rounded-2xl object-cover border-2 sm:border-[3px] border-[#777777]"
    />
  </div>
);
...
const ActionButtons = ({ emojiPickerRef, textAreaRef }) => {
  const { formData, toggleTag, toggleEmojiPicker, insertEmoji, addFiles } =
    usePostsStore();
  ...
  return (
    <div className="flex flex-row items-center gap-2 sm:gap-0">
      <PostTagButton tag={formData.tag} status="unlocked" onClick={toggleTag} />

      <label
        htmlFor="new-post-photo"
        className="hover:cursor-pointer mx-2 sm:mx-3 lg:mx-4"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
      ...
      >
        {formData.showEmojiPicker && (
          <div
            ref={emojiPickerRef}
            className="absolute -right-2 sm:-right-3 bottom-8 sm:bottom-10 z-50"
    ...
    <Flowbite theme={{ theme: MODAL_THEME }}>
      ...
      >
        <div className="py-3 sm:py-4 flex flex-col border-german-red border-2 bg-white rounded-lg transition duration-150 ease-out w-full max-w-4xl mx-auto">
          <Modal.Header className="z-10 transition ease-in-out duration-300 mb-4">
              ...
              <div className={`${FORM_CONFIG.heights.container} flex flex-col`}>
                ...
                <div className={FORM_CONFIG.styles.contentContainer}>
                  ...
                  >
                    ...
                    {attachmentsExist && (
                      <div
                        className={`${FORM_CONFIG.heights.attachmentsContainer} flex flex-row items-center gap-3 sm:gap-4 lg:gap-5 overflow-x-auto pt-2`}
                      >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mt-1">
                  <div className="flex items-center justify-start">
                    <label
                      htmlFor="new-post-choice"
                      className="me-2 text-sm sm:text-base font-medium whitespace-nowrap"
                    >
                      Send a copy
                    </label>
                    <select
