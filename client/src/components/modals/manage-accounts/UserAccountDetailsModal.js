...
const UserAccountDetailsModal = React.memo(({
  ...
  const handleSaveWithImage = async () => {
    try {
      ...
      Swal.fire({
        title: 'Error',
        text: error.response?.data?.message || 'Failed to save profile picture',
        icon: 'error',
  ...
  const hasChanges = () => {
    ...
    const imageChanged = profileImage !== null || profileImagePreview !== (data?.profilePicLink || null);
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-6 border border-gray-200">
          <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
            ...
            <div className="flex-shrink-0">
              <ImageUploadField
                label=""
                currentImage={profileImagePreview || `${formData?.firstName} ${formData?.lastName}`}
