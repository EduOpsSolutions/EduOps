...
function GradeStudentsTable({
  ...
  return (
    <div className="overflow-x-auto">
      <table className="table-auto text-base text-left rtl:text-right text-black mx-auto w-full min-w-0">
        <thead className="text-base text-black text-center border-b-dark-red-2 border-b-2">
          <tr>
            <th scope="col" className="p-2 font-normal min-w-0 text-left">
              {" "}
              ID{" "}
            </th>
            <th scope="col" className="p-2 font-normal min-w-0 text-left">
            ...
            <th scope="col" className="p-2 font-normal min-w-0">
              {" "}
              Document{" "}
            </th>
            <th scope="col" className="p-2 font-normal min-w-0">
        ...
        <tbody className="text-center">
          {students.length > 0 ? (
            students
              ...
              .map((student) => {
                ...
                const uniqueStudentKey =
                  student.user?.id || student.userId || student.studentGradeId;
                const studentId = student.user?.id || student.userId;
                const hasPendingUpload = hasPendingFile
                ...
                return (
                  <tr key={uniqueStudentKey}>
                    <td
                      className="px-2 sm:px-3 py-3 sm:py-4 text-left font-medium text-base truncate max-w-[120px]"
                      title={
                        student.user
                          ? student.user.userId
                          : student.userId || gradeId
                      }
                    >
                      {" "}
                      {student.user
                    ...
                    <td
                      className="px-2 sm:px-3 py-3 sm:py-4 text-left font-medium text-base truncate max-w-[140px]"
                      title={
                        student.user
                    ...
                    <td className="px-1 sm:px-2 py-3 sm:py-4 text-center">
                      <div className="flex flex-row gap-1 justify-center items-center">
                        <button
                          ...
                          onClick={() =>
                            hasFile && handleViewDocument(gradeId, studentId)
                          }
                          title={
                            hasPendingUpload
                              ? "View Pending File (Not Saved)"
                              : hasFile
                              ? "View Document"
                              : "No file uploaded"
                          }
                        ...
                        <button
                          ...
                          onClick={() =>
                            !isPeriodLocked &&
                            triggerFileInput(uniqueStudentKey)
                          }
                          title={
                            isPeriodLocked
                              ? "Period locked - cannot upload files"
                              : hasFile
                              ? "Replace Document"
                              : "Upload Document"
                          }
                        ...
                        <input
                          ...
                          style={{ display: "none" }}
                          ref={(el) =>
                            (fileInputRefs.current[uniqueStudentKey] = el)
                          }
                          onChange={(e) =>
                            onFileChange(uniqueStudentKey, gradeId, e)
                          }
                        />
                      </div>
                    </td>
                      ...
                      {(() => {
                        ...
                        if (localGrades && Array.isArray(localGrades)) {
                          const localGrade = localGrades.find(
                            (item) =>
                              item.studentId === student.user?.userId ||
                              item.studentId === student.userId
                          );
                          if (localGrade) {
                            // Map CSV values to UI values
                            if (localGrade.grade === "PASS") grade = "pass";
                            else if (localGrade.grade === "FAIL")
                              grade = "fail";
                            else if (localGrade.grade === "NOGRADE")
                              grade = "ng";
            ...
            <tr>
              <td
                colSpan="4"
                className="px-2 sm:px-3 py-3 sm:py-4 text-center text-gray-500 text-base"
              >
