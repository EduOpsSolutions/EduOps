...
export const getAcademicPeriods = async (req, res) => {
    try {
        const academicPeriods = await prisma.academic_period.findMany({ where: { deletedAt: null } });
        const now = new Date();
        const withStatus = academicPeriods.map(period => ({
            ...period,
            batchStatus: now < period.startAt ? 'upcoming'
                : now > period.endAt ? 'ended'
                : 'ongoing',
            enrollmentStatus: period.isEnrollmentClosed ? 'closed'
                : (now < period.enrollmentOpenAt ? 'upcoming'
                    : now > period.enrollmentCloseAt ? 'ended'
                    : 'open')
        }));
        res.json(withStatus);
    } catch (err) {
...
export const getAcademicPeriod = async (req, res) => {
    try {
        const period = await prisma.academic_period.findUnique({ where: { id: req.params.id } });
        if (!period)
    ...
    try {
        ...
        if (overlapping) {
            return res.status(400).json({
                error: "There is already an ongoing or overlapping enrollment window. Only one open enrollment period is allowed at a time."
            });
        }
