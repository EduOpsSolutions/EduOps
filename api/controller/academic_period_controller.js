...
export const getAcademicPeriod = async (req, res) => {
    try {
        const academicPeriod = await AcademicPeriod.getAcademicPeriodById(req.params.id);
        if (!academicPeriod) return res.status(404).json({ error: 'Academic Period Not found' });
...
export const updateAcademicPeriod = async (req, res) => {
    try {
        const academicPeriod = await AcademicPeriod.updateAcademicPeriod(req.params.id, req.body);
        res.json(academicPeriod);
    } catch (err) {
...
export const deleteAcademicPeriod = async (req, res) => {
    try {
        const academicPeriod = await AcademicPeriod.deleteAcademicPeriod(req.params.id);
...
export const getActiveAcademicPeriod = async (req, res) => {
    try {
        ...
        if (!academicPeriod) return res.status(404).json({ error: 'No active academic period found' });
        res.json(academicPeriod);
    } catch (err) {
